  ...
  <div>
    <div style="display: flex">
      ...
      <a-input-search
          ...
          @search="onSearch"
      />
      <a-button style="margin-left: 10px" size="large" type="information" @click="loadTableData">
        刷新
      </a-button>
    </div>
    <a-table :loading="loading" :columns="columns" :data-source="data" rowKey="id">
      <span slot="name" slot-scope="text">{{ text }}</span>
      <span slot="customTitle">商品名称</span>
      <span slot="action" slot-scope="text, record, index">
          <a-button @click="handleUpdate(record)" type="primary" icon="edit"/>
          <a-divider type="vertical"/>
          <a-button @click="handleDelete(record,index)" type="danger" icon="delete"/>
    </span>
    </a-table>
